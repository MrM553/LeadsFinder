...
### 2026-08-19 — Fixed intermittent "search finds nothing" bug

User report: "the search doesn't work all of the time." Diagnosed by querying the live production D1 database's `searches` table directly — found the exact same query ("Dachdecker München") returned 10 results in one run and 0 in a later run. Traced to a real bug in `runOverpassQuery` (`overpass.ts`): it returned immediately on *any* successful (non-throwing) response — including one with zero elements — so it never tried the fallback mirror if the primary instance returned a "successful" empty response instead of an error. Confirmed live: `overpass-api.de` does this specifically for Cloudflare-Worker-origin traffic (already knew it returns occasional 521s; now confirmed it can also return a clean 200-with-zero-elements for a query with real data). Fixed: an empty response is no longer trusted until every configured endpoint has been tried; only accepted as genuinely empty once none of them return anything. Also hardened: an error with no HTTP status (e.g. a malformed/non-JSON 200 body — reproduced live) is now retried too instead of failing the search outright.

Bumped the inter-request throttle from 2s to 3s — while diagnosing this, direct testing showed *both* configured mirrors returning empty for the same query minutes apart, which is consistent with this tool's own heavy testing today adding to their load. Considered (and deliberately did not add) a second retry round that re-tries all endpoints again after a delay when everything comes back empty — the existing auto-widen-on-empty logic in `discover.ts` already provides a second, differently-timed attempt with a different (larger) bbox, and stacking another full retry round on top risked pushing worst-case latency for a genuinely empty search close to Workers' request time limits.

4 new unit tests using mocked fetch responses to reproduce each scenario directly (86 total) — chose not to keep re-testing against the live mirrors for this fix specifically, since doing so was itself part of what was loading them down today.
